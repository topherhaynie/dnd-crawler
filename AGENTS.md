...
- Never use `has_method()` on typed objects. Protocol base classes guarantee all method stubs are present — guard on `null` only. If a method is missing from a protocol, add a stub to the protocol rather than adding a call-site guard. See `architecture.instructions.md` for the full rule and exceptions.
- **After making changes** (especially adding `class_name` declarations, adding new signals, or modifying autoloads), prompt the user to reload the project in the Godot editor so the engine picks up the new class registrations and the analyzer refreshes its type graph. Changes to `class_name` or autoload scripts in particular require a project reload before the Godot editor recognises them.
