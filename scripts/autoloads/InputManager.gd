...
func _process(_delta: float) -> void:
	for device_id in gamepad_bindings.keys():
		var player_id: int = gamepad_bindings[device_id]
		var raw := Vector2(
...
func set_vector(player_id: int, vec: Vector2) -> void:
...
func clear_vector(player_id: int) -> void:
	_vectors.erase(player_id)

## Bind a gamepad device_id to a player profile id.
func bind_gamepad(device_id: int, player_id: int) -> void:
