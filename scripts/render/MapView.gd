...
## Wall quads auto-generated by closed DOOR tokens.
## Maps token_id (String) → PackedVector2Array (4 world-space corners, rotation-aware).
var _door_wall_quads: Dictionary = {}
...
func load_map(map: MapData) -> void:
	...
	if _preg != null and _preg.token != null:
		for raw_tok in _preg.token.get_all_tokens():
			...
			if td == null:
				continue
			var half := Vector2(td.width_px, td.height_px) * 0.5
			var r := Rect2(td.world_pos - half, Vector2(td.width_px, td.height_px))
			if td.category == TokenData.TokenCategory.DOOR and td.blocks_los:
				_door_wall_quads[td.id] = _rotated_rect_quad(td.world_pos, half, deg_to_rad(td.rotation_deg))
...
func get_door_wall_quads() -> Dictionary:
	return _door_wall_quads
...
func apply_token_passthrough_state(data: TokenData) -> void:
	var changed := false
	var half := Vector2(data.width_px, data.height_px) * 0.5
	var rect := Rect2(data.world_pos - half, Vector2(data.width_px, data.height_px))
	if data.category == TokenData.TokenCategory.DOOR:
		...
		if data.blocks_los:
			_door_wall_quads[data.id] = _rotated_rect_quad(data.world_pos, half, deg_to_rad(data.rotation_deg))
		else:
			_door_wall_quads.erase(data.id)
		changed = true
...
func clear_token_passthrough(id: String) -> void:
	...
	changed = _door_wall_quads.erase(id) or changed
	if changed:
		_passthrough_version += 1
		if _map != null:
			_rebuild_wall_occluders(_map)
...
## Return 4 world-space corners of a rectangle centred at `center` with the
## given `half_size`, rotated by `angle_rad` radians around its centre.
## Corner order: TL, TR, BR, BL (before rotation).
static func _rotated_rect_quad(center: Vector2, half_size: Vector2, angle_rad: float) -> PackedVector2Array:
	var c_tl := Vector2(-half_size.x, -half_size.y).rotated(angle_rad)
	var c_tr := Vector2(half_size.x, -half_size.y).rotated(angle_rad)
	var c_br := Vector2(half_size.x, half_size.y).rotated(angle_rad)
	var c_bl := Vector2(-half_size.x, half_size.y).rotated(angle_rad)
	return PackedVector2Array([center + c_tl, center + c_tr, center + c_br, center + c_bl])
...
func _rebuild_wall_occluders(map: MapData) -> void:
	...
	for door_quad: Variant in _door_wall_quads.values():
		var door_poly: PackedVector2Array = door_quad as PackedVector2Array
		var occ_poly := OccluderPolygon2D.new()
	...
	if fog_overlay != null:
		...
		# Include closed DOOR wall quads in fog polygons.
		for door_quad: Variant in _door_wall_quads.values():
			active_polys.append(Array(door_quad as PackedVector2Array))
...
func _apply_wall_handle_drag(world_pos: Vector2) -> void:
	...
	if points.size() < 3:
		return
	# For axis-aligned rectangles, preserve opposite corner logic so the
	# four corners stay rectangular.  General 4-sided polygons (or any other
	# vertex count) just move the single dragged vertex.
	if points.size() == 4 and _is_axis_aligned_rect(points):
...
## Return true when the 4-element point array forms an axis-aligned rectangle
## (all edges are horizontal or vertical).  Order-independent.
static func _is_axis_aligned_rect(points: Array) -> bool:
	if points.size() != 4:
		return false
	for i in range(4):
		var a: Variant = points[i]
		var b: Variant = points[(i + 1) % 4]
		if not a is Vector2 or not b is Vector2:
			return false
		var av := a as Vector2
		var bv := b as Vector2
		if not is_equal_approx(av.x, bv.x) and not is_equal_approx(av.y, bv.y):
			return false
	return true
