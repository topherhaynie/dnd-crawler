...
## Passthrough rects for SECRET_PASSAGE LOS suppression.
...
## Clip a wall polygon by all active passthrough rects using exact geometry.
## Returns polygon fragments remaining after subtracting door openings.
## An empty return means the polygon is fully inside a passthrough area.
func _clip_wall_by_passthroughs(points: PackedVector2Array) -> Array[PackedVector2Array]:
	if _passthrough_rects.is_empty():
		return [points]
