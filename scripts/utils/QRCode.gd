...
static func _place_format_info(modules: Array, mask: int, size: int) -> void:
	...
	# QR spec ISO 18004 Figure 19: format information bit positions.
	# Bit index 14 = MSB, bit index 0 = LSB.
	#
	# Strip 1 — top-left finder:
	#   Row 8, col 0..5 → bits 14..9
	#   Row 8, col 7    → bit 8   (col 6 is timing, skipped)
	#   Row 8, col 8    → bit 7
	#   Col 8, row 7    → bit 6   (row 6 is timing, skipped)
	#   Col 8, row 5..0 → bits 5..0
	var strip1_row8_cols: Array[int] = [0, 1, 2, 3, 4, 5, 7, 8]
	for i: int in range(8):
		var bit: bool = ((bits >> (14 - i)) & 1) == 1
		modules[8][strip1_row8_cols[i]] = bit
	var strip1_col8_rows: Array[int] = [7, 5, 4, 3, 2, 1, 0]
	for i: int in range(7):
		var bit: bool = ((bits >> (6 - i)) & 1) == 1
		modules[strip1_col8_rows[i]][8] = bit

	# Strip 2 — top-right finder (row 8) + bottom-left finder (col 8):
	#   Row 8, cols (size-1)..(size-8) → bits 0..7
	for i: int in range(8):
		var bit: bool = ((bits >> i) & 1) == 1
		modules[8][size - 1 - i] = bit
	#   Col 8, rows (size-1) down to (size-7) → bits 14..8
	for i: int in range(7):
		var bit: bool = ((bits >> (14 - i)) & 1) == 1
