...
## Phase 1: Foundation — Project Scaffold & Dual-Process Architecture ✅
**Status:** Complete — see [docs/phase-1-implementation.md](../../docs/phase-1-implementation.md)  
**Goal:** Running skeleton with two fully independent OS windows and core autoload singletons.

> **Architecture pivot (implemented):** macOS groups all `Window` nodes from a single Godot process
> at the OS level, preventing truly independent window behavior (co-moving, shared minimize, etc.).
> Solution: two separate Godot OS processes — DM host + Player display — communicating over a local
> WebSocket connection. The DM process auto-launches the Player process via `OS.create_process()`.

1. Remove `[dotnet]` section from `project.godot`; set `embed_subwindows=false`
...
3. Create three Autoload singletons:
   ...
   - `NetworkManager.gd` — owns WebSocketMultiplayerPeer server (DM process only)
4. `Main.gd` dual-mode detection via `OS.get_cmdline_user_args()`:
   - No `--player-window` arg → DM mode: load `DMWindow.tscn`, start WS server, spawn Player process
   - `--player-window` present → Player mode: load `PlayerMain.tscn`, connect WS client
5. `PlayerClient.gd` — WebSocket client; sends `{"type":"display"}` handshake; emits `state_received`
6. `NetworkManager.gd` — classifies peers as display vs. input; `broadcast_to_displays()` helper
7. Placeholder scenes: `DMWindow.tscn`, `PlayerWindow.tscn`, `PlayerMain.tscn`
8. **Future expansion:** spawn additional per-player windows by calling `_launch_player_process()` again

**Verification:** Two fully independent OS windows open on launch (different App windows, no co-moving);
WS handshake logged on both sides; `GameState.windows` populated on DM side — **all criteria met ✅**
